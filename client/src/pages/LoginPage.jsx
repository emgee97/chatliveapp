    ...
    <div className='min-h-screen bg-cover bg-center flex items-center justify-center gap-8 sm:justify-evenly max-sm:flex-col backdrop-blur-2xl'>
...

        {
          currState === "Sign up" && isDataSubmitted && (
            ...
            rows={4} className='p-2 border border-gray-500 rounded-md focus:outline-none focus:ring-2 focus:ring-green-500'
            placeholder='provide a short bio...' required></textarea>
          )
        }

        <button type='submit' className='py-3 bg-gradient-to-r from-green-400 to-green-600 text-white rounded-md cursor-pointer'>
        ...
        <div className='flex flex-col gap-2'>
          {currState === "Sign up" ? (
            ...
            className='font-medium text-green-500 cursor-pointer'>Login here</span></p>
          ) : (
            <p className='text-sm text-gray-600'>Create an account <span onClick={()=> setCurrState("Sign up")}
            className='font-medium text-green-500 cursor-pointer'>Click here</span></p>
          )}
        </div>
