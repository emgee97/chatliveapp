...
const Sidebar = () => {
  ...
  return (
    <div className={`bg-[#8185B2]/10 h-full p-5 rounded-r-xl overflow-y-scroll text-white ${selectedUser ? "max-md:hidden" : ''}`}>
        <div className='pb-5'>
            <div className='flex justify-between items-center'>
                ...
                <div className="relative py-2 group">
                    ...
                    cursor-pointer' />
                    <div className='absolute top-full right-0 z-20 w-32 p-5 rounded-md bg-neutral-800 border border-gray-600 text-gray-100 hidden group-hover:block'> 
        ...
        <div className='bg-neutral-800/50 rounded-full flex items-center gap-2 py-3 px-4 mt-5'>
    ...
    <div className='flex flex-col mt-5'>
        {filteredUsers.map((user, index) => (
            ...
            && 'bg-neutral-800/50'}`}>
                ...
                {unseenMessages[user._id] > 0 && <p className='absolute top-4 right-4 text-cs h-5 w-5 flex justify-center items-center rounded-full bg-green-500/50'>{unseenMessages[user._id]}</p>}
            </div>
        ) )}
    </div>
